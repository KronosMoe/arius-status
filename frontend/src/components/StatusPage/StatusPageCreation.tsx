...
import { DASHBOARD_PATH, STATUS_PAGE_PATH } from '@/constants/routes'
import { z } from 'zod'
...
export default function StatusPageCreation() {
  ...
  const [logo, setLogo] = useState<string>('https://avatars.githubusercontent.com/u/5034243?v=4')
  const [selectedMonitor, setSelectedMonitor] = useState<{ id: string; type: 'card' | 'line' }[]>([])
  ...
  useEffect(() => {
    const selected: { id: string; type: 'card' | 'line' }[] = [
      ...statusCards.map((m) => ({ id: m.id, type: 'card' as const })),
      ...statusLines.map((m) => ({ id: m.id, type: 'line' as const })),
    ]
